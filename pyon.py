...
class pyon:
	...
	def deconstruct(string, *args):
		...
		for i, argument in enumerate(args):
			curent_place_in_data = list(data)[i]
			data[curent_place_in_data] = argument

		is_assignment = string[-1] == ':'
		problem = is_assignment and len(string.split()) != 2
		if problem: print('aaaah')

		output_line = private.line()
